...
Aliases""")
    ...
    async def membercount(self, ctx: commands.Context):
        ...
        embed = discord.Embed(
            ...
            description="Here is **`%s`**'s member count" %
            (ctx.guild.name),
            color=0x2f3136)
    ...
    async def poll(self, ctx: commands.Context, *, message):
        ...
        await msg.add_reaction("<:Incite_Thumbsup:1251971206412894208>")
        await msg.add_reaction("<:Incite_Thumbsdown:1251971203183415419>")
    ...
    async def users(self, ctx: commands.Context):
        embed = discord.Embed(
            title=f"**Users:**",
            description=
            f"**{len(set(self.bot.get_all_members()))} Users Of Incite**",
            color=0x2f3136)
        await ctx.send(embed=embed)
